# python 3.8.13
# potential_V2_and_S_as_parameter.py.py

# Created on Oct 10 13:44:23 2022
# @author: yinchaoyang
# --- utf-8 ---

# Solve 1D Schrodinger equation with the Potential V(x) = x^4 - x^2 using a Gaussian basis 
...
def Integral_Phi_multiply_operatorH_mutiply_Phi(si, sj):
    return (math.exp(-v * (si - sj)**2 / 2) * (8 * h_bar**2 * v**3 * (1 - v * (si - sj)**2)\
         + m * (v * ((si + sj)**2 * (v * (si + sj - 2) * (si + sj + 2) + 6)\
             - 4) + 3))) / (16 * math.sqrt(2 * math.pi) * m * v**(3 / 2))
...
if __name__ == '__main__':
    ...
    print("The 3 lowest energy eigenstates with potential V(x) = x^4 - x^2 are: ")
    for i in range(3):
