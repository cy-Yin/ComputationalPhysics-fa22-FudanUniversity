...
# Created on Oct 10 14:12:35 2022
# @author: yinchaoyang
# --- utf-8 ---
...
def Integral_Phi_multiply_operatorH_mutiply_Phi(vi, vj):
    return math.sqrt(vi * vj) * (4 * h_bar**2 * vi * vj * (vi + vj)\
         + m * (4 * (s**2 - 1) * s**2 * vi**2 + 2 * vi * (2 * s**2 * (2 * (s**2 - 1)\
             * vj + 3) - 1) + 4 * s**2 * vj * ((s**2 - 1) * vj + 3) - 2 * vj + 3))\
                 / (4 * m * (vi + vj)**(5 / 2) * math.sqrt(math.pi))
