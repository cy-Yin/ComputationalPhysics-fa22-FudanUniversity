...
# Created on Dec 11 09:32:56 2022
# version: 1.0
# @author: yinchaoyang
# --- utf-8 ---
...
# In question1, rho(x, y) = 0, phi(0, y) = phi(l_x, y) = phi(x, 0) = 0, phi(x, l_y) = 1
...
h = L/(n-1)
x = np.linspace(0, L, n)
y = np.linspace(0, 1.5*L, int(1.5 * n))
X,Y = np.meshgrid(x, y)
max_iter = 1000 # the number of iteration

# Source term
rho = np.zeros((int(1.5 * n), n))
        
# Initial
u0 = np.zeros((int(1.5 * n), n))
u0[-1,:] = 1
